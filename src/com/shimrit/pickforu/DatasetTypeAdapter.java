...
* 
*/
public class DatasetTypeAdapter extends TypeAdapter<GraphObjectEntity> {
   @Override
   public GraphObjectEntity read(JsonReader reader) throws IOException {
       // the first token is the start object
       JsonToken token = reader.peek();
       GraphObjectEntity dataset = new GraphObjectEntity();
       if (token.equals(JsonToken.BEGIN_OBJECT)) {
           reader.beginObject();
           while (!reader.peek().equals(JsonToken.END_OBJECT)) {
               if (reader.peek().equals(JsonToken.NAME)) {
                   if (reader.nextName().equals("list"))
                       dataset.setList(reader.nextString());
                   else
                       reader.skipValue();
   ...
   @Override
   public void write(JsonWriter out, GraphObjectEntity value) throws IOException {
